...
static void draw_fuel_quantity_panel(SDL_Renderer *renderer, TTF_Font *font, const EICAS_Canvas *canvas, const AircraftSystems_Data *data)
{
    ...
    const float h = 72.0f;
    const float side_qty = data->fuel_quantity * 48.7f;
    const float center_qty = data->fuel_quantity * 60.9f;
    const float total_qty = center_qty + side_qty * 2.0f;
    ...
    eicas_ui_draw_centered_text(renderer, font, canvas, EICAS_COLOR_WHITE, x + 93.0f, y + 22.0f, "%.1f", center_qty);
    eicas_ui_draw_centered_text(renderer, font, canvas, EICAS_COLOR_WHITE, x + 173.0f, y + 22.0f, "%.1f", side_qty);
...
static void draw_eicas1_page(SDL_Renderer *renderer, TTF_Font *font, const EICAS_Canvas *canvas, const AircraftSystems_Data *data)
{
    ...
    const float tat = 11.9f + 0.2f * sinf(data->simulation_time * 0.25f);

    eicas_ui_draw_text(renderer, font, canvas, EICAS_COLOR_CYAN, 54.0f, 46.0f, "TAT");
    eicas_ui_draw_text(renderer, font, canvas, EICAS_COLOR_WHITE, 151.0f, 46.0f, "%.1f C", tat);
    ...
    eicas_ui_draw_value_box(renderer, font, canvas, 60.0f, 501.0f, 60.0f, 26.0f, "%.1f", left->fuel_flow / 500.0f);
    eicas_ui_draw_value_box(renderer, font, canvas, 283.0f, 501.0f, 60.0f, 26.0f, "%.1f", right->fuel_flow / 500.0f);
